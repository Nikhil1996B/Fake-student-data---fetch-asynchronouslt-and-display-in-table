...
const displayResponseData = (data = [], displayEntity = "")=>{
    ...
    if (preElement) preElement.innerHTML = JSON.stringify(data, null, 4);
};
...
const reduceStudentsResponse = (entries, experinces)=>{
    return entries.reduce((student, current)=>{
        const findStudentExperience = experinces.find((studentExperience)=>studentExperience?.id === current.id);
        student.push({
            ...current,
            ...findStudentExperience
        });
        return student;
    }, []);
};
const displayStudentDetails = ()=>{
    Promise.all(promises).then(([entriesResponse, experienceResponse])=>{
        ...
        const studentReduced = reduceStudentsResponse(entriesResponse, experienceResponse);
